...
#dummy example of updating time disc.  
# create data for perlen, nstp, tsmult, and the steady flag
# Keeping things one layer for the time being.
perlen = [1,1] #period length
nstp = [1,200] #number of time steps per period
tsmult = [1,1] #time step multiplier
steady = [True,False] #steady state?
...
# Update UPW
##########################
# the Upstream Weight Package - UPW is required for the MFNWT solver
# Here are some example values.  These values will need to be tuned later.

# update the horizontal hydraulic condutivity (hk) using a multiplier, 
#current value is 10 m/d
ml.upw.hk *= 1.75e-03 #m/d

# update the specific storage (ss) using a multiplier, current value is 1e-06 and will need to be tuned later.
ml.upw.ss *= 1.0
...
plt.title("Rattlesnake Creek 100m modflow-nwt heads")
plt.tight_layout()
plt.show()
